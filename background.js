...
function extractPageContent(tab, config) {
  chrome.scripting.executeScript({
    ...
    openGoogleAIStudio(config.summaryPrompt, content, tab.title);
  });
}
...
function extractYouTubeTranscript(tab, config) {
  chrome.scripting.executeScript({
    ...
    openGoogleAIStudio(config.summaryPrompt, transcript, tab.title);
  });
}
...
function getYouTubeTranscript() {
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      let transcriptText = `Title: ${title}\nURL: ${url}\nVideo ID: ${videoId}\n\nTranscript:\n`;
      ...
      resolve(`Error extracting transcript: ${error.message}`);
    }
  });
}
...
function openGoogleAIStudio(prompt, content, title) {
  ...
  chrome.tabs.create({ url: 'https://aistudio.google.com/prompts/new_chat' }, (newTab) => {
    ...
    }, 3000); // Give the page 3 seconds to load
  });
}
